...
def predict_all(input_seqs, output_path, thr, thread, skip_gene_scan, skip_blast):
    ...
    for index, records in enumerate(records_list):
        if records != []:
            if index < 3:
                max_len = 500*(index+1)
                code_list = pp.encode_fasta_short(records, max_len, seq_gene_list[index])
                cnn_feature = code_list
            else:
                code_list = pp.encode_fasta_long(records, seq_gene_list[index])
