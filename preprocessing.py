...
def encode_fasta_short(record_list, max_len, gene_list, saved_path=None):
    """
    ...
    :param gene_list: a list of genes binned according to their contigs
    ...
    for i, record in enumerate(record_list):
        fw_code = encode_seq_short(record.seq, gene_list[i], word2num_dict, strand='+')
        bw_code = encode_seq_short(record.seq.reverse_complement(), gene_list[i], word2num_dict, strand='-')
...
def encode_fasta_long(record_list, gene_list, saved_path=None):
    """
    Encode a list of records storing the information of contigs with length >1.5kbp
    ...
    for i, record in enumerate(record_list):
        ...
        interval_start = 0
        while seq_len-interval_start > 1500:
            fw_inerval = [interval_start, interval_start+1500]
            fw_code = encode_seq_long(record.seq, fw_inerval, gene_list[i], word2num_dict, strand='+')
            bw_inerval = [seq_len-interval_start-1500, seq_len-interval_start]
            bw_code = encode_seq_long(record.seq.reverse_complement(), bw_inerval, gene_list[i], word2num_dict, strand='-')
        ...
        left_len = seq_len-interval_start
        omit = False
        if 100 <= left_len <= 500:
            max_len = 500
        elif 500 <= left_len <= 1000:
            max_len = 1000
        elif 1000 <= left_len <= 1500:
            max_len = 1500
        else:
            omit = True
        if not omit:
            fw_inerval = [interval_start, seq_len]
            fw_code = encode_seq_long(record.seq, fw_inerval, gene_list[i], word2num_dict, strand='+')
            bw_inerval = [0, seq_len-interval_start]
            bw_code = encode_seq_long(record.seq.reverse_complement(), bw_inerval, gene_list[i], word2num_dict, strand='-')
...
def encode_seq_short(seq, gene_infor, word2num_dict, strand):
    """
    Encode a contig with <=1500bp longth
...
def encode_seq_long(seq, interval, gene_infor, word2num_dict, strand):
    """
    Encode a seq's interval with <=1500bp longth, and the seq is longer than 1500bp
    ...
    gene_region = get_interval_gene_region(gene_region, interval)

    # encode seq's interval
    last_end = interval[0]
    seq_code = []
    for index, region in enumerate(gene_region):
        (start, end) = region
    ...
    if interval[1] > last_end:
        # encode non-coding region
        seq_code += encode_by_kmer(seq[last_end:interval[1]], NON_GENE_K, word2num_dict[0])  # 1 mer

    return seq_code


def get_interval_gene_region(gene_region, interval):
    """
    Return the gene regions of the seq's interval

    :param gene_region: all the gene_regions on a seq
    :param interval: a interval on the seq
    """
    interval_gene_region = []
    for index, region in enumerate(gene_region):
        (start, end) = region
        if end>interval[0] and start<interval[1]:
            if start>=interval[0] and end<=interval[1]:
                interval_gene_region += [region]
            elif start>=interval[0] and end>interval[1]:
                interval_gene_region += [(start, interval[1])]
            elif start<interval[0] and end<=interval[1]:
                interval_gene_region += [(interval[0], end)]
            else:
                interval_gene_region += [(interval[0], interval[1])]
    return interval_gene_region
